...
- **Serving-size rescale.** A recipe row's quantity only means something *relative to the ingredient's `serving_size`* (the trigger computes `nutrient × quantity / serving_size` and never reads the row's unit). So any **actual** `serving_size` change rescales every matching recipe row by `new_size / old_size` — stored rounded to 4 decimal places — which preserves each recipe's nutrition by construction: the same ratio cancels in the trigger math. The factor is computed from the DB's old size in a single atomic update (the unit change and size re-entry arrive in one request; no two-phase apply). Rows are matched by name, case-insensitively and trimmed, across the current user's recipes and global recipes.
...
- The guard that decides whether an ingredient update enters the recipe-sync block reads `if available != None or available is not db_ingredient.available:` (`ingredient_router.py`, update endpoint) — it looks like it should be `and`, so a PUT that only sets nutrition fields still walks the sync path (harmlessly today, since change detection keeps it inert).
- The trigger's fallback lookup (`models.py`) is not restricted to global stock: if the owning user has no ingredient with that name, it falls back to **any** user's ingredient row with the same name, so another user's values could feed a recipe's nutrition.
