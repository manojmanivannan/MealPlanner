document.addEventListener('DOMContentLoaded', () => {
    ...
    function initLayout() {
        ...
        container.innerHTML = `
            <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-2 space-y-2 sm:space-y-0 mb-4">
                ...
                <input id="ingredient-search-input" type="search" placeholder="🔍 Search ingredients..." class="clay-input border border-transparent w-full sm:w-auto mr-4" autocomplete="off">
    ...
    function displayIngredients() {
        ...
        const filtered = cachedIngredients.filter(ing => 
        ...
        if (filtered.length === 0 && query !== '') {
            html += `<div class="text-center text-stone-500 py-4">No ingredients found matching "${query}".</div>`;
        }
        ...
        listSection.querySelectorAll('input[type="checkbox"]').forEach(cb => {
            cb.addEventListener('change', async (e) => {
                ...
                try {
                    ...
                    const ing = cachedIngredients.find(i => i.id == id);
                    if (ing) {
