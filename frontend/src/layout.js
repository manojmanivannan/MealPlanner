 ...
 * ------------------------------------------------------------------
 * Material Flat Minimalist Top App Bar + Mobile Bottom Navigation + Theme Toggle + Auth Bootstrap
 */
...
const NAV = [
  {
    key: 'planner',
    label: 'Planner',
    fullLabel: 'Weekly Planner',
    href: 'index.html',
    icon: `<svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="4" width="14" height="13" rx="2"/><path d="M14 2v4M6 2v4M3 8h14"/></svg>`,
  },
  {
    key: 'recipes',
    label: 'Recipes',
    fullLabel: 'Recipe Hub',
    href: 'recipe-hub.html',
    icon: `<svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H16"/><path d="M6.5 2H16v18H6.5A2.5 2.5 0 0 1 4 17.5V4.5A2.5 2.5 0 0 1 6.5 2z"/><path d="M8 6h5M8 10h5"/></svg>`,
  },
  {
    key: 'ingredients',
    label: 'Pantry',
    fullLabel: 'Ingredients',
    href: 'ingredients.html',
    icon: `<svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M10 2a6 6 0 0 0-6 6c0 4 6 10 6 10s6-6 6-10a6 6 0 0 0-6-6z"/><circle cx="10" cy="8" r="2.5"/></svg>`,
  },
  {
    key: 'shopping',
    label: 'Shopping',
    ...
    icon: `<svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="8" cy="17" r="1.5"/><circle cx="15" cy="17" r="1.5"/><path d="M2 3h2.5l2 10h10l2-7H5.5"/></svg>`,
  },
]
...
function headerHTML(activeNav) {
  const desktopLinks = NAV.map((n) => {
    ...
    return `<a href="${n.href}" class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-all duration-150 ${activeClass}"${active ? ' aria-current="page"' : ''}>${n.icon}<span>${n.fullLabel}</span></a>`
  }).join('')
  ...
  const mobileLinks = NAV.map((n) => {
    ...
    return `<a href="${n.href}" class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-colors ${activeClass}"${active ? ' aria-current="page"' : ''}>${n.icon}<span>${n.fullLabel}</span></a>`
  }).join('')

  const bottomNavItems = NAV.map((n) => {
    const active = n.key === activeNav
    return `
      <a href="${n.href}" class="mp-bottom-nav-item ${active ? 'active' : ''}" ${active ? 'aria-current="page"' : ''}>
        <div class="mp-bottom-nav-icon-wrap">${n.icon}</div>
        <span>${n.label}</span>
      </a>`
  }).join('')
  ...
  <header class="sticky top-0 z-50 bg-surface/90 backdrop-blur-md border-b border-line transition-colors">
    <nav class="max-w-[1600px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
      <!-- Brand Logo -->
      <a href="index.html" class="flex items-center gap-2.5 flex-none focus-ring rounded-lg p-1 group">
        ...
        <div class="flex flex-col">
          ...
          <span class="text-[10px] uppercase font-semibold tracking-wider text-muted hidden sm:inline">NutriTrack</span>
        </div>
      </a>
      ...
      <div class="flex items-center gap-1.5 sm:gap-2">
        ...
        <div id="user-menu" class="relative">
          ...
          <div id="user-dropdown" class="absolute right-0 mt-2 w-48 bg-surface-raised border border-line rounded-xl shadow-lg py-1.5 hidden animate-in fade-in zoom-in-95 duration-100 z-50" role="menu">
        ...
        <button id="mobile-menu-btn" type="button" class="hidden mp-btn mp-btn-ghost mp-btn-icon focus-ring" aria-label="Open navigation menu" aria-expanded="false">
    ...
    <!-- Mobile Top Drawer (Optional) -->
    <div id="mobile-nav-drawer" class="hidden border-t border-line bg-surface px-4 py-3 space-y-1">
      ${mobileLinks}
    </div>
  </header>

  <!-- Mobile Bottom App Bar -->
  <nav class="mp-bottom-nav" aria-label="Mobile Navigation">
    ${bottomNavItems}
  </nav>`
}
...
/**
 * Mount the shared nav header + bottom bar + auth bootstrap into #app-header.
